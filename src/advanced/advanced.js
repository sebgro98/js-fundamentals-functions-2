...
function parseRequest(req) {
  const requestLines = req.trim().split('\n')

  if (requestLines.length === 0 || !requestLines[0].trim()) {
    return {
      method: '',
      path: '',
      headers: {},
  ...
  const [method, fullPath] = requestLines[0].split(' ')
  ...
  while (i < requestLines.length && requestLines[i].trim() !== '') {
    parseHeader(requestLines[i], headers)
    i++
  }

  if (i < requestLines.length - 1) {
    body = parseBody(requestLines.slice(i + 1).join(''))
  }
